...
class _FashionListScreenState extends State<FashionListScreen> {
    ...
    return Container(
      ...
      child: ListView(
        ...
        children: [
          ...
          _buildFilterChip(null, "Size", hasDropdown: true).onTap(() => _showAllFilters()),
          _buildFilterChip(null, "Color", hasDropdown: true).onTap(() => _showAllFilters()),
  ...
  Widget _buildFilterChip(IconData? icon, String label, {bool hasDropdown = false, bool isIconOnly = false}) {
    bool isActive = label != "Price" && label != "Size" && label != "Color" && !isIconOnly;

    return Container(
      margin: const EdgeInsets.only(right: 8),
      padding: const EdgeInsets.symmetric(horizontal: 12),
      decoration: BoxDecoration(
        color: isActive ? Colors.black.withOpacity(0.05) : Colors.white,
      ...
      child: Row(
        ...
        children: [
          ...
          if (!isIconOnly) label.text.size(12).semiBold.color(isActive ? Colors.black : Colors.black).make(),
          if (hasDropdown) Icon(Icons.keyboard_arrow_down, size: 16, color: isActive ? Colors.black : Colors.grey).box.padding(const EdgeInsets.only(left: 4)).make(),
        ],
      ),
    );
  }
