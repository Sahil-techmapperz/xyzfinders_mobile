...
class _FurnitureDetailScreenState extends State<FurnitureDetailScreen> {
  ...
  Widget build(BuildContext context) {
    return Consumer<ProductProvider>(
      builder: (context, provider, child) {
        ...
        if (product == null) {
          return Scaffold(
            appBar: AppBar(title: Text(widget.title ?? "Furniture Detail")),
            body: Center(child: (provider.error ?? "Furniture not found").text.make()),
          );
        }
        ...
        if (specsList.isEmpty) {
          specsList.add({"label": "Material", "value": specs['material'] ?? "Wood"});
          specsList.add({"label": "Type", "value": specs['type'] ?? "Standard"});
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          (attrs['highlights']?.toString() ?? "Elegant Design | Durable Material | Contemporary Style").text.bold.size(13).make(),
  ...
  Widget _buildAmenities(dynamic amenitiesData) {
    ...
    if (allAmenities.isEmpty) {
      return "Handcrafted quality materials used.".text.gray500.size(13).make();
    }
        ...
        child: Column(
          ...
          children: [
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                "Furniture Options".text.xl.bold.make(),
                const CloseButton(),
              ],
            ),
            const SizedBox(height: 20),
