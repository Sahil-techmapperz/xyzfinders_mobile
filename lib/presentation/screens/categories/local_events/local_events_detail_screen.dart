...
class _LocalEventsDetailScreenState extends State<LocalEventsDetailScreen> {
  ...
  Widget build(BuildContext context) {
    return Consumer<ProductProvider>(
      builder: (context, provider, child) {
        ...
        if (product == null) {
          return Scaffold(
            appBar: AppBar(title: Text(widget.title ?? "Event Detail")),
            body: Center(child: (provider.error ?? "Event not found").text.make()),
          );
        }
        ...
        if (specsList.isEmpty) {
          specsList.add({"label": "Event Date", "value": specs['date'] ?? "TBA"});
          specsList.add({"label": "Venue", "value": specs['venue'] ?? "TBA"});
          specsList.add({"label": "Organizer", "value": specs['organizer'] ?? "Multiple"});
        }
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          (attrs['highlights']?.toString() ?? "Community Event | All Ages Welcome | Fun & Engagement").text.bold.size(13).make(),
                          const SizedBox(height: 20),
                          "Event Highlights".text.bold.size(15).make(),
                          const SizedBox(height: 16),
                          _buildSpecsTable(specsList),
                          const Divider(height: 40),
                          "About Event".text.bold.size(15).make(),
                          ...
                          "Event Amenities".text.bold.size(15).make(),
                          const SizedBox(height: 16),
    ...
    return SliverAppBar(
      ...
      flexibleSpace: FlexibleSpaceBar(
        background: Stack(
          ...
          children: [
            if (images.isEmpty)
              Container(color: Colors.grey.shade100, child: const Icon(Icons.event_available, size: 50, color: Colors.grey))
            else
              PageView.builder(
  ...
  Widget _buildAmenities(dynamic amenitiesData) {
    ...
    if (allAmenities.isEmpty) {
      return "All standard event facilities available.".text.gray500.size(13).make();
    }
        ...
        child: Column(
          ...
          children: [
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                "Event Features".text.xl.bold.make(),
                const CloseButton(),
              ],
            ),
            const SizedBox(height: 20),
  ...
  Widget _buildMapView(ProductModel product) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        "Event Location".text.bold.size(15).make(),
        const SizedBox(height: 12),
        ClipRRect(
  ...
  Widget _buildSellerCard(ProductModel product) {
    return Column(
      children: [
        ...
        const SizedBox(height: 12),
        (product.sellerName ?? "Organizer").text.bold.size(16).center.make(),
